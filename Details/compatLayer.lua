...
do
	...
	local waitTable = {}
	local waitFrame = C_TimerFrame or CreateFrame("Frame", "C_TimerFrame", UIParent)
...
if buildInfo <= 30300 then -- WotLK compatLayer
  compatLayer.isWotLK = true
elseif buildInfo <= 50400 then -- MoP compatLayer
  compatLayer.isMoP = true
end

compatLayer.FSPAT = "%s*"..(gsub(gsub(FOREIGN_SERVER_LABEL, "^%s", ""), "[%*()]", "%%%1")).."$"

Private.compatEnv = setmetatable({},{
  __index = function(thetable, key)
    if compatLayer[key] then
      return compatLayer[key]
    else
      return _G[key]
    end
  end,
  __newindex = function(thetable, key, value)
	rawset(_G, key, value)
  end
})
